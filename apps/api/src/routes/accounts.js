...
export default async function accountsRoutes(app) {
  ...
  app.post("/test", async (req, reply) => {
    ...
    // IMAP test — port 993 = implicit TLS, others = STARTTLS or plain
    const client = new ImapFlow({
      ...
      // Allow self-signed on loopback (ProtonMail Bridge)
    ...
    // SMTP test — port 465 = implicit TLS, 587 = STARTTLS, others = plain
  ...
  app.post("/", async (req, reply) => {
    ...
    const imapEnc = encrypt(data.imap_pass, key);
    const smtpEnc = encrypt(data.smtp_pass, key);
    ...
    try {
      const { rows } = await query(
        ...
        [req.user.user_id, data.label, data.email_address,
         data.imap_host, data.imap_port, data.imap_user, imapEnc,
         data.smtp_host, data.smtp_port, data.smtp_user, smtpEnc]
      );
